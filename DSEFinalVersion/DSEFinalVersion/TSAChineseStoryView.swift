...
struct TSAChineseStoryView: View {
    ...
    func performAIAnalysis() {
        ...
        callAIAPI(with: prompt, isDiscussion: false)
    }
    
    // MARK: - AI 报告生成（小组讨论）- 按照TSA小六评分准则，由LLM评估
    func performAIAnalysisForDiscussion(with transcript: String) {
        ...
        let prompt = """
        你是一个专业的小学语文老师，熟悉香港小六TSA中国语文【小组讨论】评估标准。
        
        ⚠️ 重要：本次评估只针对【学生（标记为“你”或“学生”）】的个人表现，不要评估整组讨论的质量。
        - 你需要从讨论记录中找出学生说了什么
        - 如果学生完全没有发言，请给0分并说明原因
        - 评分是基于学生的实际发言内容，而非整组表现    
        学生使用的语言：\(languageHint)
        ...
        【TSA小六小组讨论个人表现评分准则】（满分100分）
        
        一、内容完整性 (40分)
        - 优秀(36-40分)：能清晰表达个人观点，内容充实，有具体例子支撑，逻辑清晰
        - 良好(28-35分)：能表达观点，内容基本完整，有一定逻辑性
        - 一般(20-27分)：观点不够清晰，内容较简单，逻辑性一般
        - 待加强(0-19分)：没有明确观点，内容空洞，逻辑混乱或没有发言
        
        二、语言表达 (30分)
        - 优秀(25-30分)：语言流畅自然，用词准确丰富，语速适中
        - 良好(19-24分)：语言基本流畅，用词恰当，偶有停顿
        - 一般(13-18分)：语言不够流畅，有较多停顿或用词不当
        - 待加强(0-12分)：语言断续，难以表达意思，或用词严重不当
        
        三、互动表现 (30分) - 重点考察是否回应他人
        - 优秀(25-30分)：能主动回应他人观点（如说“我同意你的看法”、“我补充一下”），能有效沟通，有提问引导讨论
        - 良好(19-24分)：能基本回应他人，能轮流发言，有一定互动
        - 一般(13-18分)：较少回应他人，多为单向表达自己的观点
        - 待加强(0-12分)：完全无视他人发言，自说自话，或完全没有发言
        
        【特别说明】
        - 学生发言次数越少，分数应该越低
        - 如果学生从未回应过他人（没有类似“同意”、“补充”、“我觉得也是”等回应词），互动表现分数不应高于15分
        - 如果学生完全没有发言，总分为0分
        ...
        {
            "score": 85,
            "scoreReason": "解释：学生发言X次，能清晰表达观点，并能回应同学意见如'我同意你的观点'",
            "standardStory": "小组讨论表现良好的示例：这位同学能主动发言X次，清晰表达自己的立场，并用合适的句式回应他人。",
            "completeness": {"level": "优秀/良好/一般/待加强", "comment": "具体评语"},
            "language": {"level": "优秀/良好/一般/待加强", "comment": "具体评语"},
            "creativity": {"level": "优秀/良好/一般/待加强", "comment": "具体评语"},
            "suggestions": ["具体建议1", "具体建议2", "具体建议3"],
            "encouragement": "鼓励的话"
        }
        """
        
        callAIAPI(with: prompt, isDiscussion: true)
    }
    
    // MARK: - 通用 API 调用方法
    private func callAIAPI(with prompt: String, isDiscussion: Bool = false) {
        ...
        let requestBody: [String: Any] = [
            "model": "qwen-plus",
            "messages": [
                ["role": "system", "content": "你是一个专业的小学语文老师，熟悉香港TSA评估标准。只返回纯JSON格式，不要有任何其他解释文字。"],
    ...
    // MARK: - 辅助方法：提取用户发言
    private func extractUserSpeeches(from transcript: String) -> [String] {
        var userSpeeches: [String] = []
        let lines = transcript.split(separator: "\n", omittingEmptySubsequences: false)
        
        for line in lines {
            let stringLine = String(line)
            // 格式："你: 发言内容" 或 "你：发言内容"
            if stringLine.hasPrefix("你:") || stringLine.hasPrefix("你：") {
                let colonIndex = stringLine.firstIndex(of: ":") ?? stringLine.firstIndex(of: "：")
                if let colon = colonIndex {
                    let content = String(stringLine[stringLine.index(after: colon)...]).trimmingCharacters(in: .whitespaces)
                    if !content.isEmpty {
                        userSpeeches.append(content)
                    }
                }
            }
        }
    ...
    // MARK: - 辅助方法：检查用户是否回应他人
    private func checkIfUserRespondedToOthers(in transcript: String) -> Bool {
        let lowercasedTranscript = transcript.lowercased()
        let responseKeywords = [
            "同意", "我同意", "我认为", "我觉得", "我补充", "我也认为",
            "你说得对", "我认同", "另外", "还有", "而且", "但是", "不过",
            "agree", "right", "yes", "also", "addition", "however"
        ]
        
        // 获取用户的发言内容
        let userSpeeches = extractUserSpeeches(from: transcript)
        
        for speech in userSpeeches {
            let lowercasedSpeech = speech.lowercased()
            for keyword in responseKeywords {
                if lowercasedSpeech.contains(keyword) {
                    return true
                }
            }
        }
    ...
    func useFallbackDiscussionReport() {
        // 从讨论记录中提取用户发言
        let userSpeeches = extractUserSpeeches(from: discussionTranscript)
        let userSpeechCount = userSpeeches.count
        let hasResponded = checkIfUserRespondedToOthers(in: discussionTranscript)
        
        // 根据用户发言情况计算分数
        var score = 0
        var completenessLevel = "待加强"
        var completenessComment = ""
        var languageLevel = "待加强"
        var languageComment = ""
        var interactionLevel = "待加强"
        var interactionComment = ""
        var scoreReason = ""
        
        if userSpeechCount == 0 {
            score = 0
            completenessLevel = "待加强"
            completenessComment = "学生在讨论中没有发言"
            languageLevel = "待加强"
            languageComment = "没有发言记录"
            interactionLevel = "待加强"
            interactionComment = "没有参与互动"
            scoreReason = "学生在小组讨论中没有发言，未能展示讨论能力。小组讨论评分需要评估学生的实际发言表现。"
        } else {
            // 内容完整性评分 (满分40)
            var contentScore = 0
            if userSpeechCount >= 4 {
                contentScore = 35
                completenessLevel = "优秀"
                completenessComment = "发言积极，能清晰表达观点"
            } else if userSpeechCount >= 2 {
                contentScore = 28
                completenessLevel = "良好"
                completenessComment = "能够表达基本观点"
            } else {
                contentScore = 20
                completenessLevel = "一般"
                completenessComment = "发言次数较少，观点不够充分"
            }
            
            // 语言表达评分 (满分30)
            var languageScore = 0
            let totalWordCount = userSpeeches.joined().count
            if totalWordCount > 150 {
                languageScore = 25
                languageLevel = "优秀"
                languageComment = "语言表达流畅，内容丰富"
            } else if totalWordCount > 60 {
                languageScore = 20
                languageLevel = "良好"
                languageComment = "语言表达基本流畅"
            } else {
                languageScore = 15
                languageLevel = "一般"
                languageComment = "发言内容较少，语言表达有待加强"
            }
            
            // 互动表现评分 (满分30)
            var interactionScore = 0
            if hasResponded && userSpeechCount >= 3 {
                interactionScore = 25
                interactionLevel = "优秀"
                interactionComment = "能积极回应他人观点，互动良好"
            } else if hasResponded {
                interactionScore = 20
                interactionLevel = "良好"
                interactionComment = "能够基本回应他人"
            } else if userSpeechCount >= 2 {
                interactionScore = 12
                interactionLevel = "一般"
                interactionComment = "发言多为单向，较少回应他人"
            } else {
                interactionScore = 8
                interactionLevel = "待加强"
                interactionComment = "几乎没有互动"
            }
            
            score = contentScore + languageScore + interactionScore
            scoreReason = "学生发言\(userSpeechCount)次，\(hasResponded ? "能回应他人观点" : "较少回应他人")，\(completenessComment)"
        }
        ...
        if userSpeechCount == 0 {
            ...
            suggestions = []
            if userSpeechCount < 3 {
                suggestions.append("增加发言次数，尝试更积极参与讨论")
            }
            if !hasResponded {
                suggestions.append("练习回应他人观点，可以说「我同意你的看法，我还想补充...」")
            }
        ...
        let fallbackReport = AIReportData(
            ...
            standardStory: "小组讨论表现良好的示例：这位同学能主动发言，清晰表达自己的立场，并用合适的句式回应他人。",
            completeness: AIReportData.EvaluationItem(level: completenessLevel, comment: completenessComment),
            language: AIReportData.EvaluationItem(level: languageLevel, comment: languageComment),
            creativity: AIReportData.EvaluationItem(level: userSpeechCount >= 2 ? "一般" : "待加强", comment: userSpeechCount >= 2 ? "可以尝试提出更多创新观点" : "需要更多参与"),
            suggestions: suggestions,
            encouragement: userSpeechCount == 0 ? "没关系，下次记得要发言哦！从简单的一句话开始也可以！" : "做得不错！继续练习，你会越来越自信！"
        )
